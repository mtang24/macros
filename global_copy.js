...
function plotGLRange() {
  ...
  // Define a color scale for the diabetes groups (same as in plotData).
  const color = d3.scaleOrdinal()
    .domain(["Healthy", "Pre-Diabetes", "Type 2 Diabetes"])
    .range(["#2C7BB6", "#FDB863", "#D7191C"]);

  // Define sort order for diabetes groups.
  const groupOrder = { "Healthy": 0, "Pre-Diabetes": 1, "Type 2 Diabetes": 2 };

  // Sort subjects:
  // For each subject in subjectMetricsResults, look up its Diabetes classification
  // from the global subjectData (using the first row for that subject).
  ...
  // Create a y scale (ordinal) for subjects sorted by Diabetes group.
  ...
  svg.selectAll("circle.min-gl")
    ...
    .attr("r", 5)
    // Lookup diabetes group from subjectData for consistent color coding.
    .attr("fill", d => {
